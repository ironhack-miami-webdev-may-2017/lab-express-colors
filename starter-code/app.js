...
// imports the ejs pacakege and allos use to separate view files
app.set('view engine, ejs');

// hosts all the files inside the public folder
app.use(express.static('public'));

// tells express we want to use the express-ejs-layouts package
app.use(expressLayouts);

// tells express that out layout file is pages/master-layout.ejs
app.set('layout', 'master-layout.ejs');

// app.locals.viewTitle = 'Express Views';
//
// app.locals.bodyClass = 'normal-body';
...
app.get('/colors', (request, response, next) => {
  response.render( 'colors.ejs', { viewColors: colors} );
});
...
app.get('/random-color', (request,response, next) => {
  const randomIndex = Math.floor(Math.random() * colors.length);
  response.render('random-color.ejs', {
      viewRandomColor: colors[randomIndex]
  });
});
