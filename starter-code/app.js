...
// Because we named our folder "pages" instead of views, which is the default
app.set('views','pages');

//Tells express that our view files are in the pages/ folder -
 // that way routes don't need to include the folder name
app.use(express.static('public'));

//Tells express to use Layouts
app.use(expressLayouts);
//Telling the app where to pull that file from.
// 'layout' is something that is already recognized by set, its the second parameter that has the file route
app.set('layout','master-layout.ejs');
// If you call the file just layout.ejs, then you don't need to specify the first "layout" as a paramenter
// locals are the global variables for all your pages
app.locals.pageTitle = 'Gradients.io';
...
app.get('/grayscale',(req, res, next) =>{
  res.render('grayscale-view.ejs', {
    colorsForView: colors,
    pageTitle: "Grayscales"
  });
});
...
app.get('/random-color',(req, res, next) =>{
  ...
  res.render('random-color-view.ejs', {
