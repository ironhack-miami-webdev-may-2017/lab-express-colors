...
app.get('/colors', (request,response,next) => {
  response.render('colors-view.ejs', {
...
app.get('/purple', (request,response,next) => {
  response.render('purples-colors-view.ejs', {
    colorsForView: colors
  });
});
app.get('/red', (request,response,next) => {
  response.render('red-colors-view.ejs', {
    colorsForView: colors
  });
});
app.get('/random', (request,response,next) => {
  const randomIndex = Math.floor(Math.random() * colors.length) +1;
  response.render('random-colors-view.ejs', {
    randomColorsForView: colors[randomIndex]
  });
});
