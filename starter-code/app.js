...
app.set('view engine', 'ejs'); //imports the ejs package and allows us to use view files

// app.set('views', 'pages'); // tells express that our view files are in pages/ folder

app.use(express.static('public')); // hosts all the files inside the public/ folder from localhost:3000

app.use(expressLayouts); // tells Express that we want to use the EJS layouts package

app.set('layout', 'my-master-layout.ejs'); // tells Express that our layout file is "pages/my-master-layout.ejs"


//Routes
app.get('/', (req, res, next) =>{
  res.render('homepage.ejs');  //send pages/homepage-view.ejs to the browser
});
...
app.get('/random-color', (req, res, next) =>{
  ...
  res.render('random-colors.ejs', {viewRandomColor: colors[randomIndex]});
});
app.listen(3000);
